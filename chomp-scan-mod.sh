...
NOTICA="";
CONFIG_FILE="";
TOOL_PATH="/home/vishnu/bounty/tools";
TOOL_PATH_SET=0;
...
function usage() {
                ...
                echo -e "$BLUE""\\t-L config-file \\n\\t\\t$ORANGE (optional) The path to a config file. This can be used to provide more granular control over what tools are run.""$NC";                echo -e "$BLUE""\\t-d wordlist\\n\\t\\t$ORANGE (optional) The wordlist to use for subdomain enumeration. Three built-in lists, short, long, and huge can be used, as well
as the path to a custom wordlist. The default is short.""$NC";
                echo -e "$BLUE""\\t-c \\n\\t\\t$ORANGE (optional) Enable content discovery phase. The wordlist for this option defaults to short if not provided.""$NC";
                echo -e "$BLUE""\\t-C wordlist \\n\\t\\t$ORANGE (optional) The wordlist to use for content discovery. Five built-in lists, small, medium, large, xl, and xxl can be used,
as well as the path to a custom wordlist. The default is small.""$NC";
...
function run_dnscan() {
                ...
                echo -e "$GREEN""[i]$ORANGE Command: $DNSCAN -d $1 -t 30 -o $WORKING_DIR/dnscan_out.txt -w $2.""$NC";
                START=$(date +%s);
                $DNSCAN -d "$1" -t 30 -o "$WORKING_DIR"/dnscan_out.txt -w "$2";
                END=$(date +%s);
                DIFF=$(( END - START ));
...
function run_massdns() {
                ...
                if [[ "$3" == "alone" ]]; then
                                ...
                                START=$(date +%s);
                                cat "$WORKING_DIR"/$ALL_DOMAIN "$WORKING_DIR"/$ALL_IP "$WORKING_DIR"/massdns-appended.txt | sort | uniq | $MASSDNS_BIN -r $MASSDNS_RESOLVERS -q -t A -o S
-w "$WORKING_DIR"/massdns-result.txt;
                                END=$(date +%s);
                                DIFF=$(( END - START ));
                else
                                ...
                                echo -e "$GREEN""[i]$ORANGE Command: cat (all found domains and IPs) | $MASSDNS_BIN -r $MASSDNS_RESOLVERS -q -t A -o S -w $WORKING_DIR/massdns-result.txt.""$NC";
                                START=$(date +%s);
                                cat "$WORKING_DIR"/$ALL_DOMAIN "$WORKING_DIR"/$ALL_IP "$WORKING_DIR"/goaltdns-output.txt "$WORKING_DIR"/massdns-appended.txt | sort | uniq | $MASSDNS_BIN
-r $MASSDNS_RESOLVERS -q -t A -o S -w "$WORKING_DIR"/massdns-result.txt;
                                END=$(date +%s);
                                DIFF=$(( END - START ));
                fi
...
function run_aquatone () {
                ...
                if [[ "$1" == "default" ]]; then
                                if [[ "$USE_ALL" -eq 1 ]]; then
                                                ...
                                                mkdir "$WORKING_DIR"/aquatone;
                                                echo -e "$BLUE""[i] Running aquatone against all $(wc -l "$WORKING_DIR"/"$INTERESTING_DOMAINS" | awk '{print $1}') interesting discovered
subdomains.""$NC";
                                                START=$(date +%s);
...
function run_nmap() {
                ...
                # Make sure masscan actually created output
                                ...
                                if [[ $PORT_NUMBER -gt 22000 ]]; then
                                                echo -e "$GREEN""[!]$RED WARNING: Masscan found more than 22k open ports. This is more than nmap's port argument length limit, and likely
indicates lots of false positives. Consider running nmap with -p- to scan all ports.""$NC";
                                                sleep 2;
                                                return;
                                fi
...
function run_subjack() {
                ...
                if [[ $2 == $WORKING_DIR/$ALL_RESOLVED ]]; then
                                ...
                                echo -e "$GREEN""[i]$BLUE Running subjack against all $(wc -l "$WORKING_DIR"/$ALL_RESOLVED | awk '{print $1}') discovered interesting subdomains to check
for subdomain takeover.""$NC";
                                echo -e "$GREEN""[i]$ORANGE It will run twice, once against HTTPS and once against HTTP.""$NC";
                                echo -e "$GREEN""[i]$ORANGE Command: subjack -d $1 -w $2 -v -t 20 -ssl -m -o $WORKING_DIR/subjack-output.txt""$NC";
                                START=$(date +%s);
                                "$SUBJACK" -d "$1" -w "$2" -v -t 20 -ssl -m -o "$WORKING_DIR"/subjack-https-output.txt -c "$HOME"/go/src/github.com/haccer/subjack/fingerprints.json;
